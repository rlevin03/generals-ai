"""
Train the loaded DQN agent (P0) against 3 normal greedy opponents (P1, P2, P3).
...
checkpoints/dqn_policy_vs_normal_greedy_final.pt so the original checkpoint is preserved.
"""
...
SAVE_CHECKPOINT = os.path.join("checkpoints", "dqn_policy_vs_normal_greedy_final.pt")
...
def make_greedy_callable(env: GeneralsEnv, agent: Any) -> Callable[[Any], int]:
    """Return callable(controller) -> action_index for the given greedy agent."""
...
def main() -> None:
    ...
    print(
        f"Training DQN (P0) vs 3× Normal Greedy (P1,P2,P3)\n"
    ...
    for game_id in range(NUM_GAMES):
        ...
        if (game_id + 1) % SAVE_EVERY_N_GAMES == 0:
            ...
            path = os.path.join(CHECKPOINT_DIR, f"dqn_policy_vs_normal_greedy_game_{game_id + 1}.pt")
            dqn_agent.save(path)
