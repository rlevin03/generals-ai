...
from generals import CellType  # Add this import
...
class ConvDQN(nn.Module):
    """Enhanced Convolutional DQN with reduced action space support"""
    def __init__(self, input_shape, n_actions, use_reduced_actions=True):
        ...
        if use_reduced_actions:
            # Output Q-values for each cell and each direction
            # This allows dynamic action space
...
class OptimizedGeneralsEnv(GeneralsEnv):
    """Optimized environment with reduced action space"""
...
class PrioritizedReplayBuffer:
    """Enhanced replay buffer with batch operations"""
...
class TrainingStatistics:
    """Comprehensive statistics tracking for training"""
    ...
    def log_episode(self, episode, stats):
        ...
        df.to_csv(self.episode_csv, mode='a', header=not os.path.exists(self.episode_csv), index=False)
    ...
    def log_training(self, step, stats):
        ...
        if len(self.training_stats['step']) % 100 == 0:
            ...
            df.to_csv(self.training_csv, mode='a', header=not os.path.exists(self.training_csv), index=False)
    ...
    def log_evaluation(self, episode, stats):
        ...
        # Append to CSV
        df = pd.DataFrame([stats])
        df.to_csv(self.eval_csv, mode='a', header=not os.path.exists(self.eval_csv), index=False)
    
    def get_episodes_per_hour(self, current_episode):
        """Calculate episodes per hour"""
...
class OptimizedSelfPlayDQN:
    """Optimized DQN with parallel environments and reduced action space"""
    def __init__(self, env_params, lr=5e-4, batch_size=256, num_parallel_envs=4, save_dir="checkpoints"):
        ...
        # Replay buffer
        self.replay_buffer = PrioritizedReplayBuffer(200000)  # Larger buffer
        ...
        self.past_models = deque(maxlen=20)  # Keep more past versions
        self.self_play_update_interval = 200
        
        # Statistics
        self.step_count = 0
        self.episode_count = 0
        self.writer = SummaryWriter(f'runs/generals_optimized_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    def save_checkpoint(self, episode):
        ...
        model_state = self.q_network.module.state_dict() if hasattr(self.q_network, 'module') else self.q_network.state_dict()
    ...
    def train_step(self, beta=0.4):
        ...
        with torch.no_grad():
            ...
            grad_norm = sum(p.grad.norm().item() for p in self.q_network.parameters() if p.grad is not None)
    ...
    def collect_parallel_experience(self, num_steps=100):
        ...
        # Initialize episode length tracking if not exists
        if not hasattr(self, 'env_episode_lengths'):
            ...
            for i, env in enumerate(self.env_pool.envs):
                if hasattr(env, 'game'):
                    ...
                    owned = sum(1 for row in game.grid for cell in row if cell.owner == self.env_params['player_id'])
        ...
        for step in range(num_steps):
            ...
            for i in range(self.num_parallel_envs):
                ...
                elif hasattr(self, 'invalid_move_counts') and self.invalid_move_counts[i] > 500:
                    force_end = True
                ...
                if dones[i]:
                    ...
                    elif hasattr(self.env_pool.envs[i], 'game') and hasattr(self.env_pool.envs[i].game, 'winner'):
    ...
    def update_self_play_opponents(self):
        """Update opponents for self-play"""
...
def train_optimized():
    ...
    while trainer.episode_count < total_episodes:
        ...
        # Update self-play opponents
        if trainer.episode_count % trainer.self_play_update_interval == 0 and trainer.episode_count > 0:
